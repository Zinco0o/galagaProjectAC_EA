...
public abstract class Entity {
    ...
    public Entity(double x, double y, double length) {
     ...
     public Boolean collision(Projectile p){
        ...
        double rayonProjectile = p.getLength() / 2;

      // Si c'est le missile du Boss et qu'il est en mode explosion
      if (p instanceof MissileAmeliorer) {
         MissileAmeliorer ma = (MissileAmeliorer) p;
         if (ma.isEstExplose()) {
            rayonProjectile = 0.08; // Rayon beaucoup plus large pour l'explosion
         }
      }
        return distance < (this.length /2 + rayonProjectile);
